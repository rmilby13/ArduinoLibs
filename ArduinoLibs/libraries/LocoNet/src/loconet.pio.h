...
#if !PICO_NO_HARDWARE
static const struct pio_program loconet_rx_program = { .instructions = loconet_rx_program_instructions, .length = 5,
        .origin = -1, };

static inline pio_sm_config loconet_rx_program_get_default_config( uint offset ) {
...
#if !PICO_NO_HARDWARE
static const struct pio_program loconet_tx_program = { .instructions = loconet_tx_program_instructions, .length = 5,
        .origin = -1, };

static inline pio_sm_config loconet_tx_program_get_default_config( uint offset ) {
	pio_sm_config c = pio_get_default_sm_config ();
