...
LocoNet::LN_SW_REQ::LN_SW_REQ( LNPacket& packet) : LNPacket (packet) {

}

arduino::String LocoNet::LN_SW_REQ::toString() {
	return LNPacket::toString() + "Requesting Switch  at " + arduino::String(this->getAddr()) + " to " + (this->getClosed()? "Closed" : "Thrown") + " Output " + this->getActive() ;
}
